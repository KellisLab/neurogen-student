...
We refer to these an allelic series because each type of mouse in the series contains an allele of *Htt* with a different polyQ length.
...
Rather than have features as rows, it takes them as columns, which causes considerable slowdown in program performance R uses column-major ordering.
...
Next we cluster the samples using one or several of many potential distance metrics to identify outliers based on transcriptomic distance.
...
The results is a tree that has been "cut" into multiple sub-trees (clusters).
...
Numeric values can be categorical, where order and value has no meaning (e.g. sex, batch, disease group), or ordinal, in which the order is relevant and the numeric values can be expected to correlate with our data (e.g. age, marker expression, time point).
...
We will use the generated plot to select a candidate powers based on when the curve appears to saturate.
...
We will choose 6, a nice, round, small number for which the scale-free topology fit index curve starts to flattens out upon reaching a reasonable high value (in this case 0.7).
...
The matrix is then heirarchically clustered to define partitioned "gene modules".

The remaining preset parameters are unimportant are determine the memory usage and run time of the algorithm.
...
net = blockwiseModules(datExpr, power = 6, corType = "bicor", maxPOutliers =  0.05, 
                        ...
                        maxBlockSize = 55000, pamRespectsDendro = FALSE)
...
In this analysis we would like to identify modules that are significantly associated with the traits or variables of ineterest.
...
Now let's use these conventient wrapper functions provided by the package to generate a heatmap of our module-trait relationships.
...
The numbers in each cell are correlations and *p*-values.
In the above Module-trait associations, each row corresponds to a module eigengene, column to a trait.
...
You can play around with the code and visualize those that have a non-significant *p*-values or poor correlations, or see how the same modules correlate with other non-relevant traits.
...
The `signedKME` function will compute the "eigengene-based connectivity", i.e. the module membership of each indivisual gene, and assign it a score.
...
We want to filter out undesirable genes and other junk, so `gene_metadata` is justa data frame of information about each gene that we will use for this purpose.
...
You might ask what purposea heatmap where all the values are pretty much the same actually serves.
...
From kME distribution on each module, we can set threshold for modules that are associated with our trait: We have 'pink', 'magenta', and 'brown' modules that are associated with `Q_length`.
...
We'll search for known marker genes of striatal spiny projection neurons (SPN), the most affected cell type in HD, and see if any of them are present inthis module.
Prior literature suggest that these genes may encompass HD vulnerability factors or have activity that is modulated by Q length.
